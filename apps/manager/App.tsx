...
import EnergyBossCommand from './components/EnergyBossCommand';
...
import { AuthProvider, ROLE_LEVELS, useAuth } from './contexts/AuthContext';
...
const Logo = () => {
  return (
    <div className="flex flex-col items-center justify-center py-6 px-4">
      <img
        src="/ana-energy-jtz-logo-horizontal-white-green.png"
        alt="ANA Energy"
        ...
        <div className="p-4 bg-dark-800 border-t border-dark-700">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3 px-2 pt-2 border-t border-dark-700">
              <div className="w-9 h-9 rounded-full bg-brand-600 border border-dark-600 flex items-center justify-center text-xs font-bold shadow-lg text-white">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 truncate">{currentUser?.role || 'Team Member'}</p>
              </div>
            </div>
          </div>
        </div>
...
const RoleBasedRedirect: React.FC = () => {
  ...
  const target = roleLevel >= ROLE_LEVELS.Manager ? "/" : "/cockpit";
                      ...
                      <Route
                        ...
                        element={
                          <ProtectedRoute>
                            <MainLayout>
                              <Routes>
                                {/* All authenticated users (Technician=40+) */}
                                ...
                                {/* Manager+ routes (80+) */}
                                <Route path="/" element={<RoleRoute minRole={ROLE_LEVELS.Manager}><Dashboard /></RoleRoute>} />
                                <Route path="/command" element={<RoleRoute minRole={ROLE_LEVELS.Manager}><EnergyBossCommand /></RoleRoute>} />
                                <Route path="/customers" element={<RoleRoute minRole={ROLE_LEVELS.Manager}><CustomerList /></RoleRoute>} />
                                <Route path="/customers/:id" element={<RoleRoute minRole={ROLE_LEVELS.Manager}><CustomerDetail /></RoleRoute>} />
                                <Route path="/workflow" element={<RoleRoute minRole={ROLE_LEVELS.Manager}><ImportManager /></RoleRoute>} />
                                <Route path="/analytics" element={<RoleRoute minRole={ROLE_LEVELS.Manager}><TerritoryAnalytics /></RoleRoute>} />

                                {/* Admin-only routes (100) */}
                                <Route path="/admin" element={<RoleRoute minRole={ROLE_LEVELS.Admin}><AdminDashboard /></RoleRoute>} />

                                {/* Default redirect: manager+ → dashboard, others → cockpit */}
