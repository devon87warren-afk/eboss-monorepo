...
const MainLayout: React.FC<MainLayoutProps> = ({ children, rightPanelContent }) => {
  ...
  const initials = displayName === 'Unknown User' || !displayName.trim()
    ? '?'
        ...
        <nav className="flex-1 p-4 overflow-y-auto">
          {canAccess(ROLE_LEVELS.Manager) && (
            <SidebarSection title="Command">
              <SidebarLink to="/command" icon={Activity} label="Command Center" />
              <SidebarLink to="/" icon={LayoutDashboard} label="Dashboard" />
            </SidebarSection>
          )}

          <SidebarSection title="Field Operations">
            <SidebarLink to="/cockpit" icon={Gauge} label="Cockpit" />
          ...
          {canAccess(ROLE_LEVELS.Manager) && (
            <SidebarSection title="Imports & Territory">
              <SidebarLink to="/workflow" icon={Upload} label="Import Manager" badge={23} />
              <SidebarLink to="/analytics" icon={MapPin} label="Territory Analytics" />
            </SidebarSection>
          )}
        ...
        {isMobileMenuOpen && (
          <div className="absolute top-16 left-0 w-full h-[calc(100%-4rem)] bg-navy-900/98 backdrop-blur-md z-40 p-4 flex flex-col md:hidden">
            <nav className="flex-1 space-y-1">
              {canAccess(ROLE_LEVELS.Manager) && (
                <>
                  <SidebarLink to="/command" icon={Activity} label="Command Center" onClick={() => setIsMobileMenuOpen(false)} />
                  <SidebarLink to="/" icon={LayoutDashboard} label="Dashboard" onClick={() => setIsMobileMenuOpen(false)} />
                  <div className="h-px bg-navy-700 my-3" />
                </>
              )}
              <SidebarLink to="/cockpit" icon={Gauge} label="Cockpit" onClick={() => setIsMobileMenuOpen(false)} />
              ...
              <SidebarLink to="/resources" icon={BookOpen} label="Knowledge Base" onClick={() => setIsMobileMenuOpen(false)} />
              {canAccess(ROLE_LEVELS.Admin) && (
                <>
                  <div className="h-px bg-navy-700 my-3" />
