...
const Resources: React.FC = () => {
  ...
  const SPEC_SHEETS = [
    { title: 'EBOSS 48V Hybrid Spec Sheet', model: 'EB-48V-150', size: '2.4 MB', date: '2024-01-15' },
    { title: 'EBOSS 100kVA System Specs', model: 'EB-100K-HYB', size: '3.1 MB', date: '2023-11-20' },
    { title: 'Battery Bank BMS Specifications', model: 'BMS-Gen4', size: '1.8 MB', date: '2024-02-10' },
    { title: 'Solar Input Controller Manual', model: 'SC-5000', size: '4.5 MB', date: '2023-09-05' },
  ];

  const SCHEMATICS = [
    { title: 'Main DC Bus Wiring Diagram', type: 'Wiring', id: 'DWG-001', icon: Zap },
    { title: 'Inverter Control Loop Schematic', type: 'Logic', id: 'SCH-104', icon: CircuitBoard },
    { title: 'Generator Auto-Start Harness', type: 'Wiring', id: 'DWG-008', icon: FileText },
    { title: 'Hybrid Transfer Switch Layout', type: 'Layout', id: 'LAY-202', icon: Battery },
  ];

  const TROUBLESHOOTING = [
    {
      id: 'ts-1',
      issue: 'Inverter Fault: "DC Over-Voltage"',
      steps: [
        'Check battery bank voltage with multimeter (Expected: 300-700VDC).',
        'Verify over voltage condition on trending data gathered.',
        'Verify disconnect switch is fully engaged.',
      
      ]
    },
    {
      id: 'ts-2',
      issue: 'Generator Fails to Auto-Start',
      steps: [
        'Check "Remote Start" toggle is ON in EBOSS controller.',
        'Verify 2 wire start state (open/closed)',
        'Inspect 12V starter battery voltage.',
        'Check for E-300 fault codes on the generator ECU.'
      ]
    },
    {
      id: 'ts-3',
      issue: 'System Not Switching to Battery Mode',
      steps: [
        'Ensure load demand is below Inverter limit (check kW meter).',
        'Verify battery State of Charge (SOC) is > 30%.',
        'Check "Hybrid Mode" timer settings in firmware settings page.',
        'Inspect Battery connected to bus (Orange Key).'
      ]
    }
  ];
