...
export function useTripProposals(
  ...
  const query = useQuery({
    ...
    retry: (failureCount, error) => {
      if (error instanceof TripServiceError && error.code === 'NOT_CONFIGURED') {
        return false;
      }
  ...
  const isUsingMockData =
    query.error instanceof TripServiceError && query.error.code === 'NOT_CONFIGURED';
...
export function useCurrentTripProposal(): {
  ...
  const currentTrip = trips.find((t) => t.status === 'proposed') ?? trips[0] ?? null;
